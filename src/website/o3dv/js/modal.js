...
export class Modal
{
    constructor ()
    {
        this.modalDiv = DomUtils.CreateDiv ('ov_modal');
    ...
    Open ()
    {
        this.overlayDiv = DomUtils.AddDiv (document.body, 'ov_modal_overlay');
...
export class ProgressDialog extends Dialog
{
    ...
    Init (text)
    {
        ...
        DomUtils.AddDiv (contentDiv, 'ov_progress_img', '<svg><use href="assets/images/3dviewer_net_logo.svg#logo"></use></svg>');
        this.textDiv = DomUtils.AddDiv (contentDiv, 'ov_progress_text');
        this.SetText (text);
    }
...
export class ButtonDialog extends Dialog
{
    ...
    Init (title, buttons)
    {
        ...
        {
            let buttonDiv = DomUtils.AddDiv (buttonsDiv, 'ov_button ov_dialog_button', button.name);
            if (button.subClass) {
        ...
        DomUtils.AddDiv (contentDiv, 'ov_dialog_title', title);
        let dialogContentDiv = DomUtils.AddDiv (contentDiv, 'ov_dialog_content');
        let buttonsDiv = DomUtils.AddDiv (contentDiv, 'ov_dialog_buttons');
        let buttonsInnerDiv = DomUtils.AddDiv (buttonsDiv, 'ov_dialog_buttons_inner');
...
export class ListPopup extends PopupDialog
{
    ...
    Init (positionCalculator)
    {
        ...
        this.listDiv = DomUtils.AddDiv (contentDiv, 'ov_popup_list ov_thin_scrollbar');
        return contentDiv;
    }

    AddListItem (item, callbacks)
    {
        let listItemDiv = DomUtils.AddDiv (this.listDiv, 'ov_popup_list_item');
        if (item.icon) {
            Utils.AddSvgIconElement (listItemDiv, item.icon, 'left_inline');
        }
        if (item.color) {
            let iconDiv = DomUtils.AddDiv (listItemDiv, 'ov_popup_list_item_icon');
            let colorCircle = Utils.CreateInlineColorCircle (item.color);
            iconDiv.appendChild (colorCircle);
        }
        DomUtils.AddDiv (listItemDiv, 'ov_popup_list_item_name', item.name);
        listItemDiv.addEventListener ('click', callbacks.onClick);
        if (Utils.IsHoverEnabled () && callbacks.onHoverStart && callbacks.onHoverStop) {
